...
import { loadCatalog } from "./entities.js";
...
const btnTrainBack = document.getElementById("btn-train-back");
const btnToBattle = document.getElementById("btn-to-battle");
...
const btnFinishOk = document.getElementById("btn-finish-ok");
const btnRetry = document.getElementById("btn-retry");
const btnMenu = document.getElementById("btn-menu");
const btnResultTrain = document.getElementById("btn-result-train");
...
const screens = [titleScreen, trainSelectScreen, supportDeckScreen, resultScreen];
...
function buildUnitButtons() {
  ...
  for (const ch of catalog.characters) {
    ...
    btn.innerHTML = `
      <img src="${ch.poses.idle.file}?v=4" alt="${ch.name}" draggable="false" />
...
function buildTrainRoster() {
  ...
  for (const ch of catalog.characters) {
    ...
    card.innerHTML = `
      <img src="${ch.poses.idle.file}?v=4" alt="${ch.name}" />
...
function renderDeckUI() {
  ...
  for (const card of SUPPORT_CARDS) {
    ...
    `;
    btn.addEventListener("click", () => {
      if (selected) deckIds = deckIds.filter((x) => x !== card.id);
      else if (deckIds.length < MAX_SUPPORTS) deckIds.push(card.id);
...
function setPortrait(pose = "idle") {
  ...
  document.getElementById("train-portrait").src = file + "?v=4";
}
...
function showTrainResult(res) {
  ...
  pendingFinished = res.finished || null;
...
function startCareerWithDeck() {
  career = new BloomCareer(trainee, deckIds);
...
btnTrainBack.addEventListener("click", () => {
  show(titleScreen);
  if (game) game.state = "title";
});

btnDeckBack.addEventListener("click", () => {
  show(trainSelectScreen);
});
...
btnTrainAbort.addEventListener("click", () => {
  career = null;
  buildTrainRoster();
  show(trainSelectScreen);
});

btnTrainFinish.addEventListener("click", () => {
  career = null;
  buildTrainRoster();
  buildUnitButtons();
  show(trainSelectScreen);
...
btnRest.addEventListener("click", () => {
  ...
  audio.ensure();
  applyActionResult(career.rest());
});

btnOuting.addEventListener("click", () => {
  if (!career || career.done) return;
  audio.ensure();
  applyActionResult(career.recreate());
});

btnResultOk.addEventListener("click", () => {
  resultOverlay.classList.add("hidden");
  if (pendingFinished) {
    showFinishFanfare(pendingFinished);
    pendingFinished = null;
  }
  renderCareer();
});
...
btnResultTrain.addEventListener("click", () => {
  hud.classList.add("hidden");
  game.state = "train";
  buildTrainRoster();
  show(trainSelectScreen);
...
window.addEventListener("game:hud", (e) => refreshButtons(e.detail));
